...
def build_melspectrogram_from_acc(acc_magnitude,sr, n_fft, hop_length, n_mels):
    """
    acc_x = raw_data[:, 0]
    acc_y = raw_data[:, 1]
    acc_z = raw_data[:, 2]
...
librosa.display.specshow(melspectrogram, sr=sr, hop_length=hop_length, x_axis='time', y_axis='mel')
plt.colorbar(format='%+2.0f dB')
plt.title('Log-Mel Spectrogram (10 Hz Sampling Rate)')
plt.xlabel('Time (s)')
